...
scenario_text() {
    case "$1" in
        retry-client)
            printf '%s' "Add exponential backoff retry support to the HTTP client and update tests accordingly."
            ;;
        parser-edge-case)
...
start_all() {
    ...
    echo -e "  ${DIM}Then run:${RESET} ./run.sh fire retry-client"
    echo ""
}
...
fire_task() {
    ...
    tailbus -socket "$sock" fire task-orchestrator "$payload"
}
...
case "${1:-start}" in
    ...
    *)
        cat <<EOF
Usage: ./run.sh [start|stop|logs|doctor|dashboard|scenarios|fire|fire-task]
EOF
        exit 1
        ;;
esac
